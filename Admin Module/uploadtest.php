...
{	
	if(count($data->sheets[$i][cells])>0) // checking sheet not empty
	{
		...
		{ 
			...
			//$marks = mysqli_real_escape_string($connection,$data->sheets[$i][cells][$j][8]);
if($_SESSION['tname']!=null)
{
	$tname=$_SESSION['tname'];
			$query = "insert into testques (Qid,tname,Ques,A,B,C,D,Answer) values ('','".$tname."','".$Ques."','".$A."','".$B."','".$C."','".$D."','".$Answer."')";
...
{
	$_SESSION['tname1']="Session is Expired";
...
if($r!=0)
{
	...
	$neg=$_SESSION['neg'];
	$time=$_SESSION['time'];
	$q="INSERT INTO testinfo (tname,tdes,neg,tim) VALUES ('$tname','$des','$neg','$time'); ";
	mysqli_query($connection,$q);
	
$_SESSION['upload']='Test Question Inserted in database';
...
unlink($temp); 
	...
	$_SESSION['upload']='Test Question not inserted in database';
